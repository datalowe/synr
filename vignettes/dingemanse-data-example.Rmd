...
The data used here are from a study by Cuskley et al. (2019) and are available through [this GitHub repository](https://github.com/mdingemanse/colouredvowels). Note that the user (mdingemanse) who has the repository is not involved in synr development, so please don't send questions about synr to him or other co-authors of the article.
...
We'll use `tidyr` to reformat the raw data, explained below, and `ggplot2` for producing custom plots. 
