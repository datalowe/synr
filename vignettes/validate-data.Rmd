...
This approach of using DBSCAN together with 3D 'variance' scores is, so far, unique to synr. Thus, there are no norms regarding what particular values should be used.
...
If you do use synr for data validation, when reporting the results, __please reference synr__ itself (for now, ) and include code and/or values you passed to `check_valid_get_twcv_scores`. Remember also that it's important to avoid post-hoc definitions of validation criteria, whenever possible. Thus, if it's feasible, __please make sure to decide before starting data collection, or based on a smaller initial sample, what values to use with `check_valid_get_twcv_scores`__.
