...
2. DBSCAN is applied to find clusters of color points.
3. The 'variance' of each cluster is calculated.
4. The TWCV is calculated.
4. If a single low-variance cluster, ie a 'tight-knit' cluster, includes more than __60%__ of all color points, the participant data are classified as invalid.
...
* `safe_num_clusters` Minimum number of identified DBSCAN clusters (including 'noise' cluster; TODO update with info about when noise cluster is included after implementation has been changed) that guarantees validity of a participant's data if points are 'non-tight-knit'.
...
## Summary
synr uses a novel approach for validating participant data which shows great promise as a tool for data cleaning. It is likely to reduce time spent on monotonous work with sorting through consistency test data. Most of all however, it lets researchers clearly specify what they consider to be valid data and how they formalized these criteria. A downside is that this validation procedure involves more abstract concepts like 'total within-cluster variance', but the benefit of not having to rely on familiar but diffuse concepts like 'number of colors' hopefully weighs greater.

Note that when synr classifies participant data as 'invalid', it essentially means that it's inappropriate to simply feed the data into consistency scoring algorithms. It's then up to you as a user to decide how to handle 'invalid' data sets, eg by excluding them from analyses entirely, or by considering participants with 'invalid' data as non-synesthetes.

Some particular response patterns only make sense to humans and don't register at all with methods like DBSCAN. This includes things like using an __o__range color for the grapheme 'O', which was found in one study (Simner et al., 2006) to be done more frequently by self-reported non-synesthetes with low consistency scores than by self-reported synesthetes. Keep in mind that if you want to partly base validity assessments on these very subjective patterns (as in e. g. Carmichael et al., 2015), synr always needs to be complemented by human ratings.

If you use synr for data validation __please reference synr__ itself (for now, https://github.com/datalowe/synr) and include code and/or values you passed to `check_valid_get_twcv_scores` when reporting the results. Remember also that it's important to avoid post-hoc definitions of validation criteria, whenever possible. Thus, if it's feasible, __please decide before starting data collection, or based on a smaller initial sample, what values to use with `check_valid_get_twcv_scores`__.
