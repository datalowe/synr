...
#' within a specified color range. Color range is specified using EITHER color_label OR
#' r/g/b arguments. Possible color_label specifications are: "blue", "red", "green",
...
#' range 0-1 (e.g. c(0.2, 0.8) ).
#'
#' Supported color spaces are: "XYZ", "sRGB", "Apple RGB", "Lab", and "Luv". Note
#' that RGB values must be specified using the range 0-1, not 0-255.
#'
...
is_in_color_range <- function(color_to_check, color_label = NULL,
  ...
  if (!is.null(color_label)) {
    if (color_label == "blue") {comp_vals <- c(0, 0.314, 0, 0.314, 0.745, 1)}
    else if (color_label == "red") {comp_vals <- c(0.745, 1, 0, 0.314, 0, 0.314)}
    else if (color_label == "green") {comp_vals <- c(0, 0.314, 0.745, 1, 0, 0.314)}
    else if (color_label == "black") {comp_vals <- c(0, 0.129, 0, 0.129, 0, 0.129)}
    else if (color_label == "white") {comp_vals <- c(0.871, 1, 0.871, 1, 0.871, 1)}
    else if (color_label == "hazy") {comp_vals <- c(0.392, 0.588, 0.392, 0.588, 0.392, 0.588)}
  } else {comp_vals <- c(r, g, b)}
  pri_rgb <- grDevices::convertColor(color_to_check, from=color_space, to="sRGB")
