...
#' @param response_times A numeric vector. Times from presentation to response, in order.
#' @param response_colors A character vector. Response colors, as hex color codes.
#' @param color_space_spec A one-element character vector. What color space is to be used? The
#' following color spaces are supported: "XYZ", "sRGB", "Apple RGB", "Lab", and "Luv"
#' @examples
#' create_grapheme(symbol="a", response_times=c(2.3, 6.7, 0.4),
#' response_colors=c("84AE99", "9E3300", "000000"), color_space_spec="Luv")
#' @export
create_grapheme <- function(symbol, response_times, response_colors, color_space_spec) {
